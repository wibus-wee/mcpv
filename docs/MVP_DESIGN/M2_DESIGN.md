...
- 引入弹性与健康：Idle 回收、minReady 预热、stateful/persistent/singleton 保护、ping 健康探测。
...
- Scheduler：实例表 + 状态机（Ready/Busy/Draining/Failed/Stopped），stateful 绑定、maxConcurrent 硬限制、minReady 维护、idleSeconds 驱动回收（定时扫描）。
...
- 测试：单元 + 集成（假 MCP server echo+ping），覆盖超限、初始化失败、ping 失败、idle 回收、stateful/persistent/singleton 行为。
...
- 错误码映射：并发超限 → `busy` (`-32002` 自定义)；启动中/无实例 → `starting`; stateful 绑定满载 → `busy`; 下游错误透传 JSON-RPC error。
...
- 数据结构：`instances map[serverType][]*trackedInstance`，`sticky map[serverType]map[routingKey]*trackedInstance`（用于 stateful 绑定）。
- Acquire：
  - stateful: 命中实例且 BusyCount<maxConcurrent → markBusy；否则若满载 → 返回 busy。
  - 非 stateful：优先 Ready 且 BusyCount<maxConcurrent；否则启动新实例（并发保护，后续可用 semaphore）。
  - markBusy: BusyCount++，State=Busy，LastActive=now。
- Release：BusyCount--，BusyCount=0→State=Ready，更新 LastActive。
- IdleManager：定时扫描（ticker），对非 persistent/singleton 且 IdleSeconds 超时的 Ready 实例 → Draining → StopInstance；stateful 需无有效绑定才可回收；MinReady：每个 serverType 至少保留 N 个 Ready（不回收）。
...
- 单元：
  ...
  - scheduler: stateful 绑定、MaxConcurrent、idle 回收（借助手动调用 IdleManager）、minReady 保温。
